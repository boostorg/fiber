...
#ifndef BOOST_TASKLETS_FUTURE_HPP
#define BOOST_TASKLETS_FUTURE_HPP
...
#include <boost/tasklet/detail/future_traits.hpp>
...
{
    typedef typename tasklets::detail::future_traits<T>::storage_type storage_type;
    typedef typename tasklets::detail::future_traits<T>::source_reference_type source_reference_type;
    typedef typename tasklets::detail::future_traits<T>::rvalue_source_type rvalue_source_type;
    typedef typename tasklets::detail::future_traits<T>::move_dest_type move_dest_type;
    ...
    void mark_finished_with_result_internal(source_reference_type result_)
		{
			tasklets::detail::future_traits<T>::init(result,result_);
        mark_finished_internal();
    }
    void mark_finished_with_result_internal(rvalue_source_type result_)
    {
        tasklets::detail::future_traits<T>::init(result,static_cast<rvalue_source_type>(result_));
        mark_finished_internal();
    }
...
class unique_future
{
    ...
    typedef typename tasklets::detail::future_traits<R>::move_dest_type move_dest_type;
...
//         template <class Allocator> explicit promise(Allocator a);
	...
	void set_value(typename tasklets::detail::future_traits<R>::source_reference_type r)
	{
	    lazy_init();
	    boost::lock_guard<mutex> lock(future->mtx);
	    if(future->done)
...
//         void set_value(R && r);
	void set_value(typename tasklets::detail::future_traits<R>::rvalue_source_type r)
	{
	    lazy_init();
	    boost::lock_guard<mutex> lock(future->mtx);
	    if(future->done)
	    ...
	    future->mark_finished_with_result_internal(static_cast<typename tasklets::detail::future_traits<R>::rvalue_source_type>(r));
	}
